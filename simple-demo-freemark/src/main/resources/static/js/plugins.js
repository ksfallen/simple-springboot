...
var DIALOG = {
    ...
    show: function (options) {
        ...
        this.box
            ...
            .css({
                "width":       opts.width,
                "margin-left": (this.fixed ? 0 : sl) - opts.width / 2,
                "margin-top":  (this.fixed ? 0 : st) - this.box.height() / 2,
                "left":        "50%",
                "top":         "50%"
            }).show();
...
DRAGBOX.prototype = {
    ...
    loadFlashSwf :function (divId, width, height, options) {
        if (divId == null || divId == undefined) {
            return;
        }
        var flashContent = "flashContent_" + divId;
        document.getElementById(divId).innerHTML = '<div id=' + flashContent + '></div>';
        var swfVersionStr = "10.0.0";
        var xiSwfUrlStr = "/swf/playerProductInstall.swf";
        var flashvars = options;
        var params = {};
        params.quality = "high";
        params.bgcolor = "#00000000";
        params.wmode = "transparent";
        params.allowscriptaccess = "always";
        params.allowfullscreen = "false";
        var attributes = {};
        attributes.id = "ImageTran";
        attributes.name = "ImageTran";
        swfobject.embedSWF("/swf/ImageTrans.swf", flashContent, width, height, swfVersionStr, xiSwfUrlStr, flashvars, params, attributes);
    },
    ...
    init :function (options) {
        ...
        if ($('#' + options['parent']).length > 0) {
            ...
            if (options['showType'] == 'Flash') { //Flash方式展示图片
                $('body').append('<div id="' + options['parent'] + '" class="dragBox"><div id="' + options['inside'] + '"  class="inside"><h1 id="' + options['locked'] + '" onselectstart="return false;">' + (options['title'] ? options['title'] : '照片大图') + '<a class="span" href="javascript:void(0);"></a></h1><div id="imgs"></div><iframe class="anyChatIframe"></iframe></div>');
                $('#' + options['parent']).css({"visibility": "hidden"});
                var w = 740;
                var h = 425;
                setTimeout(function () {
                    $('#' + options['parent']).css({"width": w, "marginLeft": -w / 2 + 'px', "marginTop": -h / 2 + st + 'px', "visibility": "visible"})
                }, 300)
                this.loadFlashSwf("imgs", w, h, options);
            }else{
                $('body').append('<div id="' + options['parent'] + '" class="dragBox" ><div id="' + options['inside'] + '"  class="inside"><h1 id="' + options['locked'] + '" onselectstart="return false;">' + (options['title'] ? options['title'] : '照片大图') + '<a class="span" href="javascript:void(0);"></a></h1><img id="imgs" style="max-width:512px;max-height:384px;" src="' + options['imgPath'] + '"></div><iframe class="anyChatIframe"></iframe></div>');
                $('#' + options['parent']).css({"visibility" :"hidden"})
                setTimeout(function () {
                    ...
                    if(h<600)
	                {
	                    ...
	                    $('#'+options['parent']).css({"width":$('#'+options['inside']).find("img").width(),"marginLeft":-w/2+'px',"marginTop":0,"visibility":"visible","top":0})
	                }
